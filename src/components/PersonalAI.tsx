  ...
  return (
    <section id="ai" className="mx-auto max-w-4xl px-6 py-20">
      ...
      <p className="mt-3 max-w-xl text-[15px] text-paperdim">
        ...
        confirmed yet, it will say so instead of guessing.
      </p>
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-border bg-[#0E1219]">
        ...
        <div ref={scrollRef} className="kb-scrollbar flex h-[420px] flex-col gap-4 overflow-y-auto px-4 py-5">
          {messages.map((m) =>
            m.role === "user" ? (
              ...
              <AssistantBubble key={m.id} content={m.content} animate={m.id === lastAssistantId} />
            )
          )}

          {loading && (
            <div className="flex max-w-[85%] items-center gap-1 rounded-2xl rounded-tl-sm border border-border bg-surface px-4 py-3">
        ...
        >
          <input
            ...
            placeholder="Ask about Sunanda, his family, education, projects..."
